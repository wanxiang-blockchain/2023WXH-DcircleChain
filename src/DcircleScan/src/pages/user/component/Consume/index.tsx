...
export default function Consume({address, upddateInfo}: {address:string, upddateInfo: (address:string) => void}) {
    ...
    useEffect(() => {
        getUs().nc.addEvent(DBDIDArticleRole.ChangedEvent, async (event) => {
            ...
            if (event.source != 'user_consume') return;
    ...
    const loadTx = async (address:string) => {
        ...
        request.size = 10;
        request.startTxTime = (txList.length > 0 && txList[txList.length - 1].buyTime) || new Date().getTime();
        request.role = DIDRole.Consumer;
    ...
    const loadStatFromServer = async (address:string) => {
        ...
        request.address = address;
        request.role = DIDRole.Consumer;
        await GetUserStatByDIDRole(request, 'user_consume');
    }
    ...
    return (
      <>
          <div className={styles.tableContainer}>
              <div className={classNames([styles.tableWrap, styles.fourWrap])}>
                  <Table rowKey="dataUpdateTime" scroll={{ x: true }} columns={columnsStat} dataSource={[statList]} pagination={false} />
              </div>
              <div className={classNames([styles.tableWrap, styles.fourWrap])}>
                  <Table size={"small"} rowKey="buyTime" scroll={{ x: true }} columns={columnsTx} dataSource={txList} pagination={false} />
              </div>
          </div>
          { !noMore && <Spin/> }
      </>
